//Interface for WorldCup Tournament
...
//Variant for slots
type SlotVariant = {slot_1:string} | {slot_2:string}
...
function generateMatchesInRoundRobin(teams:string[]){
    const dummyTeam = 'dummy'
        ...
        for(let j=0;j<numberOfTeams/2;j++){
            ...
            if(teams[j]!==dummyTeam && teams[k]!==dummyTeam){
                ...
                const match = {
                    ...
                    firstTeam:teams[j],
                    secondTeam:teams[k],
                    matchOpponents:teams[j] + ' vs ' +teams[k]
                }
...
function generateMatchDateAndSlot(saturdayDate:Date,sundayDate:Date){
    let matchList = []
    totalMatchesList.forEach((matchElement: string) => {
        matchList.push(matchElement.match)
    })
    
    let matchDate:any = []
    let originalNumberOfTeams:number = 0
    tournament.teams.forEach((team:string) =>{
        if(team === 'dummy')
    ...
    if(originalNumberOfTeams>=8){
        ...
        totalMatchesList.forEach((match:Match) =>{
